...
- From the correlation matrix, we observe the following:
    - The strongest linear correlation is between `sale_price` and `house_square_feet`. This indicates a direct linear relationship between these variables.
    - However, other potential associations, such as those involving binary variables (`has_view_quality`), are not captured here. For such cases, alternative methods (e.g., logistic regression or proportions analysis) are more appropriate.
    - The correlation matrix quantifies the linear relationships between numeric variables. As it only captures linear dependencies, it may not fully describe associations involving categorical or binary variables.
...
Based on the data from graphs A - C, sale price can be predicted to a certain extent using variables such as house square footage, year built, and number of bedrooms.
    ...
    House Square Footage: There is a moderate association (Pearson correlation ~0.53) between house square footage and sale price. As square footage increases, the sale price generally rises, although there are outliers, particularly at higher square footage values—that slightly reduce this correlation.

    Year Built: The association between year built and sale price is weaker (Pearson correlation ~0.23). However, certain years (such as homes built in the 1880s and 1930s) tend to have higher sale prices compared to other periods. This suggests that while newer homes may have some impact, specific historical periods or architectural styles may also influence prices.

    Number of Bedrooms: There is a weak association between the number of bedrooms and sale price (Pearson correlation ~0.21). While more bedrooms generally correlate with a higher price, there are notable outliers, such as homes with 0 or a very high number of bedrooms, which distort the trend.
...
```{R, echo=FALSE, message=FALSE}
# row and col propotions
housing <- housing %>%
  mutate(price_bins = cut(sale_price, breaks = 5))

cont_table_d <- table(housing$has_view_quality, housing$price_bins)

row_props_d <- prop.table(cont_table_d, margin = 1)
col_props_d <- prop.table(cont_table_d, margin = 2)

kable(row_props_d, caption = "{D3} Row Proportions of Sale Price by View Quality")
kable(col_props_d, caption = "{D4} Column Proportions of View Quality by Sale Price")
```


```{R fig.align="center", fig.width = 14, echo=FALSE, message=FALSE}
cont_df_d <- as.data.frame(as.table(cont_table_d))
names(cont_df_d) <- c("ViewQuality", "PriceBin", "Count")

cont_df_d <- cont_df_d %>%
  group_by(PriceBin) %>%
  mutate(RowProp = Count / sum(Count))

ggplot(cont_df_d, aes(x = PriceBin, y = RowProp, fill = as.factor(ViewQuality))) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "{D5} Row Proportions of Sale Price by View Quality",
       x = "Sale Price (Binned)",
       y = "Proportion",
       fill = "View Quality")
```
...
- Row proportions {D5} show how sale price is distributed across different levels of view quality.
...
# row and col propotions
housing <- housing %>%
  mutate(sq_ft_bins = cut(sq_ft_thousand, breaks = 10))

cont_table_d <- table(housing$has_view_quality, housing$sq_ft_bins)

row_props_d <- prop.table(cont_table_d, margin = 1)
col_props_d <- prop.table(cont_table_d, margin = 2)

kable(row_props_d, caption = "{E4} Row Proportions of Square Footage (in thousands) by View Quality")
kable(col_props_d, caption = "{E5} Column Proportions of View Quality by Square Footage (in thousands)")
```


```{R fig.align="center", fig.width = 14, echo=FALSE, message=FALSE}
cont_df <- as.data.frame(as.table(cont_table_d))
names(cont_df) <- c("ViewQuality", "SqFtBin", "Count")

cont_df <- cont_df %>%
  group_by(ViewQuality) %>%
  mutate(RowProp = Count / sum(Count))

ggplot(cont_df, aes(x = SqFtBin, y = RowProp, fill = as.factor(ViewQuality))) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "{E6} Row Proportions of Square Footage by View Quality",
       x = "Square Footage (Binned)",
       y = "Proportion",
       fill = "View Quality")
```
...
- The bar plot {E6} illustrates the row proportions of square footage bins within each category of `has_view_quality`. This approach provides a clearer view of association, as it shows how the numeric variable is distributed relative to the binary variable. It does not quantify linear dependence but highlights patterns in proportions that would be missed by correlation metrics.
...
```{R, echo=FALSE}
cont_table_e <- table(housing$has_view_quality, housing$has_waterfront)

row_props_e <- prop.table(cont_table_e, margin = 1)
col_props_e <- prop.table(cont_table_e, margin = 2)

kable(row_props_e, caption = "{F2} Row Proportions of Waterfront by View Quality")
kable(col_props_e, caption = "{F3} Column Proportions of View Quality by Waterfront")
```


```{R, echo=FALSE}
cont_df_e <- as.data.frame(as.table(cont_table_e))
names(cont_df_e) <- c("ViewQuality", "Waterfront", "Count")

cont_df_e <- cont_df_e %>%
  group_by(ViewQuality) %>%
  mutate(RowProp = Count / sum(Count))

ggplot(cont_df_e, aes(x = Waterfront, y = RowProp, fill = as.factor(ViewQuality))) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "{F4} Row Proportions of Waterfront by View Quality",
       x = "Waterfront",
       y = "Proportion",
       fill = "View Quality")
```
